...
sap.ui.define([
	...
	return BaseController.extend("AAUFARNameSpace.ZFORM_ODATA_AUFAR.controller.Update", {
		...
		onSwitch: function() {
			...
			// let state = this.getView().byId('switch').getState();
			// let test = this.getView().getControlsByFieldGroupId("myInputs");
			// if (state === true) {
			// 	test.forEach(enabledFunction);
			// } else {
			// 	test.forEach(disabledFunction);
			// }

			// function enabledFunction(item, index) {
			// 	if (item.setEnabled()) {
			...
			// function disabledFunction(item, index) {
			// 	if (item.setEnabled()) {
